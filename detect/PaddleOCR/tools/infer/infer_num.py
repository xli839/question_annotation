...
def load_page_number_dict(json_path):
    ...
    if isinstance(data, dict) and "image_path" in data:
        data = [data]
    return {item["image_path"]: str(item["page_number"]) for item in data}


def main(args):
    page_number_dict = load_page_number_dict("/hpc2hdd/home/xli839/lxy/work/page_num/dataset/dataset_info.json")
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            # 新增：只取图片最下方1/8部分
            h = img.shape[0]
            crop_img = img[int(h*9/10):, :, :]  # 裁剪最下方1/8 math: 17/18 chinese: 14/15
            starttime = time.time()
            ...
            for text, score in rec_res:
                ...
                if nums and score > best_score:
                    best_num = nums[0]  # 如果有多个数字，只取第一个
                    best_score = score
            pred_page_num = best_num
            # 获取真实页码
            gt_page_num = page_number_dict.get(image_file, None)
            if gt_page_num is not None:
                total += 1
                if pred_page_num == str(gt_page_num):
                    correct += 1
    ...
    # 统计准确率
    if total > 0:
        print(f"页码识别准确率: {correct}/{total} = {correct/total:.2%}")
    else:
        print("没有找到可比对的页码数据")
